...
const actions = {
	...
	fetchBannerNews({
		...
		return http.post(api.bannerNewsApi, state.bannerNews.request)
			.then(data => {
	...
	fetchOneNews({
		...
		return http.get(api.detailApi + newsId)
			.then(data => {
	...
	fetchTypeList({
		...
		return http.post(api.typesApi, state.types.request)
			.then(data => {
